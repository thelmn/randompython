...
def get_text_from_blocks(block_ints, message_length, block_size=DEFAULT_BLOCK_SIZE):
    ...
    for block_int in block_ints:
        ...
        for i in range(block_size-1, -1, -1):  # iterate backwards with step 1 to (not including) -1
            ...
            if len(message) <= message_length:  # if the decoded message is still less than expected length
                ...
                # print("ascii_number = %d" % ascii_number)

        # slice out extra bytes if current message length is larger than needed message length -- hack of the year awards go to...
        current_message_length = len(message)+len(block_message)

        # print("current_message_length = %d" % current_message_length)
        if current_message_length > message_length:
            actual_bytes = message_length - current_message_length + DEFAULT_BLOCK_SIZE
            message.extend(block_message[:actual_bytes])
            # print('is_greater by %d' % actual_bytes)
        else:
...
def encrypt_and_write_file(message, filename, block_size=DEFAULT_BLOCK_SIZE):
    ...
    if key_size < block_size * 8:  # convert block size to BITS
        ...
        for i in range(len(encrypted_blocks)):
            encrypted_blocks[i] = str(hex(encrypted_blocks[i]))
        encrypted_content = ','.join(encrypted_blocks)

        encrypted_content = '%s_%s_%s' % (hex(len(message)), hex(block_size), encrypted_content)
...
def read_file_and_decrypt(filename):
    ...
    message_length = int(message_length, 16)
    ...
    if key_size < block_size * 8:  # convert block size to BITS
        ...
        for block in encrypted_content.split(','):
            encrypted_blocks.append(int(block, 16))
...
secrets of government and inform the people." -Hugo Black'''
    ...
    # make key file for initial run. comment it out so as not to replace file
    # make_key_files(filename, key_size)
    print(encrypt_and_write_file(message, filename))
    decrypted = read_file_and_decrypt(filename)
    print(decrypted)
    fo = open('%s_plain.txt' % filename, 'w')
    fo.write(decrypted)
    fo.close()
